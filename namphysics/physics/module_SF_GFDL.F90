...
!--------------------------- LOCAL VARS ------------------------------
   ...
   WRITE(message,*)'WITHIN THE GFDL SCHEME, NTSFLG=1 FOR GFDL SLAB  2010 UPGRADS',NTSFLG
    ...
    kflip=lm  !NMMB input z is from top to bottom, !wang

   DO J=jms,jme

      DO i=ims,ime
       if( lpr .and. i == (ims+ime)/2 .and. j == (jms+jme)/2 ) then
...
!        QSURF(I)=QSFC(I,J)
        ...
        tstrc(i)=TSK(i,j)
        T1(I) = T3D(i,j,kflip)
        tpc(1,i)=T3D(i,j,kflip)
        U1(I) = U3D(i,j,kflip)
        upc(1,i)=U3D(i,j,kflip) * 100.
        USTAR(I) = UST(i,j)
        V1(I) = V3D(i,j,kflip)
        vpc(1,i)=v3D(i,j,kflip) * 100.
        Z0RL(I) = ZNT(i,j)*100.
        zoc(i)=ZNT(i,j)*100.
...
!       also divide by 10**4 to convert from /m**2 to /cm**2
    ...
    !        write(0,*)'maybe first timestep'
            zhalf = -R*tpc(1,i)*alog(pkmax(i)/pspc(i))/grav
   ...
   !
       ...
       if( lpr .and. i == (ims+ime)/2 .and. j == (jms+jme)/2 ) then
        ...
        write(0,*)isltyp(i,j),wetc(i),PSFC(i,j),Q1(I),SLIMSK(i),TSKIN(i),T1(I),U1(I),USTAR(I),V1(I),gsw(i,j),glw(i,j)
       WRITE(0,*)'i,j,upc(1,i),vpc(1,i),tpc(1,i),rpc(1,i),     &
...
!-----------------------------------------------------------------------
      ...
      real, intent ( in), dimension (lm     , ims :ime ) :: upc
      real, intent ( in), dimension (lm     , ims :ime ) :: vpc
      real, intent ( in), dimension (lm     , ims :ime ) :: tpc
      real, intent ( in), dimension (lm     , ims :ime ) :: rpc

...
!+---+-----------------------------------------------------------------+
      SUBROUTINE mp_gt_driver(qv, qc, qr, qi, qs, qg, ni, nr, &
                              ...
                              ims,ime, jms,jme, kts,kte,d_ss,mprates)

      implicit none

!..Subroutine arguments
      INTEGER, INTENT(IN):: D_SS, &
                            ims,ime, jms,jme, kts,kte
      REAL, DIMENSION(ims:ime, jms:jme,kts:kte,d_ss) :: mprates 
      REAL, DIMENSION(ims:ime, jms:jme, kts:kte), INTENT(INOUT):: &
                          qv, qc, qr, qi, qs, qg, ni, nr, th
      REAL, DIMENSION(ims:ime, jms:jme, kts:kte), OPTIONAL, INTENT(INOUT):: &
                          re_cloud, re_ice, re_snow
...
!aligo
     ...
     &                qc1d_R8, nc1d_R8, qi1d_R8, ni1d_R8, qs1d_R8
      REAL, DIMENSION(ims:ime, jms:jme):: pcp_ra, pcp_sn, pcp_gr, pcp_ic
...
!..Local variables
      ...
      INTEGER:: IRTN,MYPE,rc,nunit,mpi_comm_comp
      LOGICAL:: opened
...
!Ratko - 2013/09/12 - If existing lookup table read coefficients
      ...
      if(mype==0) then
        ...
        open(nunit,file='qr_acr_qg',form='unformatted',status='old',iostat=ierr)
        if(ierr == 0) then
          read(nunit)tcg_racg
          read(nunit)tmr_racg
          read(nunit)tcr_gacr
      ...
      CALL MPI_BCAST(tcg_racg,SIZE(tcg_racg),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tmr_racg,SIZE(tmr_racg),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tcr_gacr,SIZE(tcr_gacr),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tmg_gacr,SIZE(tmg_gacr),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tnr_racg,SIZE(tnr_racg),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tnr_gacr,SIZE(tnr_gacr),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(ierr,1                 ,MPI_INTEGER         ,0,MPI_COMM_COMP,IRTN)

!rv - if file exist, don't calculate coefficients, just return
      if(ierr==0) return
...
!Ratko - 2013/09/12 - If existing lookup table read coefficients
      ...
      if(mype==0) then
        ...
        open(nunit,file='qr_acr_qs',form='unformatted',status='old',iostat=ierr)
        if(ierr == 0) then
          read(nunit)tcs_racs1
          read(nunit)tmr_racs1
          read(nunit)tcs_racs2
          read(nunit)tmr_racs2
          read(nunit)tcr_sacr1
          read(nunit)tms_sacr1
          read(nunit)tcr_sacr2
          read(nunit)tms_sacr2
          read(nunit)tnr_racs1
          read(nunit)tnr_racs2
          read(nunit)tnr_sacr1
          read(nunit)tnr_sacr2
          close(nunit)
        endif
      endif

      CALL MPI_BCAST(tcs_racs1,SIZE(tcs_racs1),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tmr_racs1,SIZE(tmr_racs1),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tcs_racs2,SIZE(tcs_racs2),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tmr_racs2,SIZE(tmr_racs2),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tcr_sacr1,SIZE(tcr_sacr1),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tms_sacr1,SIZE(tms_sacr1),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tcr_sacr2,SIZE(tcr_sacr2),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tms_sacr2,SIZE(tms_sacr2),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tnr_racs1,SIZE(tnr_racs1),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tnr_racs2,SIZE(tnr_racs2),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tnr_sacr1,SIZE(tnr_sacr1),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(tnr_sacr2,SIZE(tnr_sacr2),MPI_DOUBLE_PRECISION,0,MPI_COMM_COMP,IRTN)
      CALL MPI_BCAST(ierr,1                   ,MPI_INTEGER         ,0,MPI_COMM_COMP,IRTN)
...
!
      IMPLICIT NONE
!
      INTEGER, INTENT(IN):: ims,ime, jms,jme, kts,kte

      REAL, DIMENSION(ims:ime,jms:jme,kts:kte), INTENT(IN):: qv,p,t,rho
      REAL, DIMENSION(ims:ime,jms:jme,kts:kte), INTENT(INOUT)::         &
                                                            qc,qi,qs,ni

      REAL, DIMENSION(ims:ime,jms:jme), INTENT(IN):: XLAND,gridkm

      REAL, DIMENSION(ims:ime,jms:jme,kts:kte), INTENT(OUT):: cldfra
...
!..Local vars.
      ...
      REAL, DIMENSION(ims:ime,jms:jme,kts:kte):: qvsat
...
!.. RH over ocean required as compared to over land.
      ...
      !write(*,*) "===== jms, jme ====", jms, jme
      !write(*,*) "===== ims, ime ====", ims, ime

      DO j = jms,jme
      DO k = kts,kte
      DO i = ims,ime

         RH_00L = 0.781 + SQRT(1./(35.0+gridkm(i,j)*gridkm(i,j)*gridkm(i,j)*0.5))
         RH_00O = 0.831 + SQRT(1./(70.0+gridkm(i,j)*gridkm(i,j)*gridkm(i,j)*0.5))

...
!
!  HISOTRY LOG
!   2016-06-22  Weiguo Wang, move gfspbledmf(for hurr) from HWRF to NMMB
!                            2016 version HWRF

MODULE module_BL_GFSPBLEDMFHUR

CONTAINS

!-------------------------------------------------------------------          
   SUBROUTINE GFSPBLEDMFHUR(U3D,V3D,TH3D,T3D,QV3D,QC3D,QI3D,P3D,PI3D,     &
...
!     REAL,  DIMENSION(ims:ime, jms:jme, lm     ), INTENT(OUT) :: DKU3D,DKT3D  
...
!! note, Z is int level, with (LM+1) in Z direction                                         
     REAL,    DIMENSION(ims:ime, jms:jme, lm+1     ), INTENT(IN) ::Z
     ...
     REAL,    DIMENSION(ims:ime, jms:jme, lm     ), INTENT(INOUT) ::   &
                                        RTHBLTEN,                       &
                                        RQCBLTEN,                       &
                                        RQIBLTEN,                       &
                                        RQVBLTEN,                       &
...
!--------------------------- LOCAL VARS ------------------------------
      ...
      REAL     (kind=kind_phys), DIMENSION(ims:ime, lm     ) ::         &
                                        DEL,                            &
                                        DU,                             &
                                        DV,                             &
                                        PHIL,                           &
      ...
      REAL     (kind=kind_phys), DIMENSION(ims:ime, lm+1     ) ::       &
                                        PHII,                           & 
                                        PRSI

      REAL     (kind=kind_phys), DIMENSION(ims:ime, lm     , 3) ::      &
                                        Q1,                             &
                                        RTG
...
!
      ...
      DO k=1,lm
        ...
        DO i=ims,ime 
          ...
          thraten(i,k) = RSWTT(I,J,KFLIP) + RLWTT(I,J,KFLIP)    !!0.0
      ...
      DO i=ims,ime 
        DEL(i,lm)=DEL(i,ktem)
        PRSI(i,ktep)=PRSI(i,lm)-DEL(i,ktem)
        PHII(I,KTEP)=PHII(I,lm)+dz8w(i,j,KX+1-lm)*G
        PHIL(I,lm)=PHII(I,lm)-PHIL(I,KTEM)+PHII(I,lm)
      ENDDO
...
!-----------------------------------------------------------------------
      ...
      END MODULE module_BL_GFSPBLEDMFHUR

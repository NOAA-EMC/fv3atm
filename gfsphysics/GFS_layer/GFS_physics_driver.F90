...
!       lprnt = kdt >=  1  .and. abs(grid%xlon(i)*rad2dg-269.15) < 0.201  &
!                          .and. abs(grid%xlat(i)*rad2dg-57.28) < 0.201
...
!  --- ...  transfer soil moisture and temperature from global to local variables
      ...
      if (Model%frac_grid) then
        do k=1,Model%kice
          do i=1,im
            if (icy(i)) then
              stice(i,k)  = max(timin, min(Sfcprop%tiice(i,k), tgice))
            endif
          enddo
        enddo
      endif
...
!           ' zorl3=',zorl3(ipr,:),' uustar3=',uustar3(ipr,:),' stice=',stice(ipr,:)
...
!    &,' stice=',stice(ipr,:),' tsfc33=',tsfc3(ipr,3),' islmsk=',islmsk(ipr)
...
!       write(0,*)' me=',me,' mintice=',minval(tice),' maxtice=',maxval(tice),' kdt=',kdt
!       if (lprnt) write(0,*)' bef sice stice=',stice(ipr,:)
        call sfc_sice                                                            &
!  ---  inputs:
           (im, lsoil, Statein%pgr,                                              &
...
!       if (lprnt) write(0,*)' sft sice stice=',stice(ipr,:)
        if (Model%frac_grid) then
          do i = 1, im
...
! Three-way composites (fields from sfc_diff)
          txl   = frland(i)              ! land fraction
          wfrac = one - txl              ! water fraction (ocean/lake)
          txi   = fice(i) * wfrac        ! txi = ice fraction wrt whole cell
...
!         Sfcprop%tprcp(i)  = txl*tprcp3(i,1)  + txi*tprcp3(i,2)  + txo*tprcp3(i,3)
          ...
          if (.not. flag_cice(i) .and. islmsk(i) == 2) then
            evap(i)         = txl*evap3(i,1)   + wfrac*evap3(i,2)
            hflx(i)         = txl*hflx3(i,1)   + wfrac*hflx3(i,2)
            qss(i)          = txl*qss3(i,1)    + wfrac*qss3(i,2)
            gflx(i)         = txl*gflx3(i,1)   + wfrac*gflx3(i,2)
          else
          ...
          if (k == 2) then
            ...
            if (flag_cice(i)) then
              if (wet(i) .and. fice(i) > Model%min_seaice) then  ! this was already done for lake ice in sfc_sice
                ...
                Sfcprop%snowd(i) = txi * snowd3(i,2)
              endif

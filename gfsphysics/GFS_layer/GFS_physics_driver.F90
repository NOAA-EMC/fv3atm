...
!  --- ...  xw: transfer ice thickness & concentration from global to local variables
        ...
        fice(i)    = min(one,Sfcprop%fice(i)) ! sea/lake ice fraction wrt water portion, not whole cell
...
! DH* In CCPP, this is in GFS_surface_composites_pre
      if (Model%frac_grid) then
        do i = 1, IM
          ...
          if (frland(i)+fice(i)*(one-frland(i)) < one) wet(i)=.true. !there is some open ocean/lake water!
        enddo
      else
        do i = 1, IM
          frland(i) = zero

    ...
    initalize_stochastic_physics: if (.not. is_initialized) then
      ...
      if ( GFS_Control%lndp_type == 2 ) then
          ...
          do v = 1,GFS_Control%n_var_lndp
            ...
            endselect 
          enddo
          if (.not. allocated(slc))    allocate(slc   (0,0,0))
          if (.not. allocated(stype))  allocate(stype (0,0))
          if (.not. allocated(stc))    allocate(stc   (0,0,0))
          if (.not. allocated(vfrac))  allocate(vfrac (0,0))
          if (.not. allocated(alnsf))  allocate(alnsf (0,0))
          if (.not. allocated(alnwf))  allocate(alnwf (0,0))
          if (.not. allocated(snoalb)) allocate(snoalb(0,0))
          if (.not. allocated(semis))  allocate(semis (0,0))
          if (.not. allocated(zorll))  allocate(zorll (0,0))
      elseif ( GFS_Control%lndp_type == 1 ) then ! this scheme sets perts once

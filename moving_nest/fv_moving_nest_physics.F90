...
contains
    ...
    do nb = 1,Atm_block%nblks
      ...
      do ix = 1, blen
        ...
        if (move_physics) then
          ...
          mn_phys%ssu(i,j)    = IPD_Data(nb)%Sfcprop%ssu(ix)
          mn_phys%ssv(i,j)    = IPD_Data(nb)%Sfcprop%ssv(ix)
      ...
      do nb = 1,Atm_block%nblks
        ...
        do ix = 1, blen
          ...
          if (move_physics) then
            ...
            if (nint(IPD_data(nb)%Sfcprop%slmsk(ix)) .eq. 0 .and. mn_phys%ssu(i,j) .gt. 1e6) then
              IPD_Data(nb)%Sfcprop%ssu(ix)  = 0.0
            else
              IPD_Data(nb)%Sfcprop%ssu(ix)  = mn_phys%ssu(i,j)
            endif
            if (nint(IPD_data(nb)%Sfcprop%slmsk(ix)) .eq. 0 .and. mn_phys%ssv(i,j) .gt. 1e6) then
              IPD_Data(nb)%Sfcprop%ssv(ix)  = 0.0
            else
              IPD_Data(nb)%Sfcprop%ssv(ix)  = mn_phys%ssv(i,j)
            endif
  ...
  subroutine mn_phys_fill_nest_halos_from_parent(Atm, IPD_Control, IPD_Data, mn_static, n, child_grid_num, is_fine_pe, nest_domain, nz)
    ...
    if (move_physics) then
      ...
      call fill_nest_halos_from_parent_masked("ssu", mn_phys%ssu, interp_type_lmask, Atm(child_grid_num)%neststruct%wt_h, &
          Atm(child_grid_num)%neststruct%ind_h, &
          x_refine, y_refine, &
          is_fine_pe, nest_domain, position, mn_phys%slmsk, 0, 0.0D0)
      call fill_nest_halos_from_parent_masked("ssv", mn_phys%ssv, interp_type_lmask, Atm(child_grid_num)%neststruct%wt_h, &
          Atm(child_grid_num)%neststruct%ind_h, &
          x_refine, y_refine, &
          is_fine_pe, nest_domain, position, mn_phys%slmsk, 0, 0.0D0)
  ...
  subroutine mn_phys_fill_intern_nest_halos(moving_nest, IPD_Control, IPD_Data, domain_fine, is_fine_pe)
    ...
    if (move_physics) then
      ...
      call mn_var_fill_intern_nest_halos(mn_phys%ssu, domain_fine, is_fine_pe)
      call mn_var_fill_intern_nest_halos(mn_phys%ssv, domain_fine, is_fine_pe)
  ...
  subroutine mn_phys_shift_data(Atm, IPD_Control, IPD_Data, n, child_grid_num, wt_h, wt_u, wt_v, &
    ...
    if (move_physics) then
      ...
      call mn_var_shift_data(mn_phys%zorlw, interp_type, wt_h, Atm(child_grid_num)%neststruct%ind_h, &
          delta_i_c, delta_j_c, x_refine, y_refine, is_fine_pe, nest_domain, position)
      call mn_var_shift_data(mn_phys%ssu, interp_type, wt_h, Atm(child_grid_num)%neststruct%ind_h, &
          delta_i_c, delta_j_c, x_refine, y_refine, is_fine_pe, nest_domain, position)
      call mn_var_shift_data(mn_phys%ssv, interp_type, wt_h, Atm(child_grid_num)%neststruct%ind_h, &
          delta_i_c, delta_j_c, x_refine, y_refine, is_fine_pe, nest_domain, position)
  ...
  subroutine mn_phys_dump_to_netcdf(Atm, Atm_block, IPD_Control, IPD_Data, time_val, file_prefix, is_fine_pe, domain_coarse, domain_fine, nz)
    ...
    real, allocatable, dimension(:,:) :: ssu_pr_local, ssv_pr_local
    ...
    if (move_physics) then
      ...
      allocate ( ssu_pr_local(is:ie, js:je) )
      allocate ( ssv_pr_local(is:ie, js:je) )
    endif
    ...
    do nb = 1,Atm_block%nblks
      ...
      do ix = 1, blen
        ...
        if (move_physics) then
          ...
          ssu_pr_local(i, j) = IPD_data(nb)%Sfcprop%ssu(ix)
          ssv_pr_local(i, j) = IPD_data(nb)%Sfcprop%ssv(ix)
    ...
    if (move_physics) then
      ...
      call mn_var_dump_to_netcdf(ssu_pr_local, is_fine_pe, domain_coarse, domain_fine, position, time_val, Atm%global_tile, file_prefix, "SSU")
      call mn_var_dump_to_netcdf(ssv_pr_local, is_fine_pe, domain_coarse, domain_fine, position, time_val, Atm%global_tile, file_prefix, "SSV")
    ...
    if (move_physics) then
      ...
      deallocate(ssu_pr_local, ssv_pr_local)

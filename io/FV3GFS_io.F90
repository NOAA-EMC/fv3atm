...
!----------------------------------------------------------------------
  subroutine sfc_prop_restart_read (Sfcprop, Atm_block, Model, fv_domain, warm_start, ignore_rst_cksum)
    ...
    integer :: nvar_emi, nvar_dust12m, nvar_gbbepx, nvar_s2me, nvar_s2l
...
!$omp parallel do default(shared) private(nb, ix)
    ...
    ! A fortran 2003 compliant compiler will call clm_lake_final automatically here
...
! Flake
        if(Model%lkm > 0 .and. Model%iopt_lake==Model%iopt_lake_flake) then
             ...
             sfc_var2(i,j,nvar2me+2) = Sfcprop(nb)%T_ice(ix)

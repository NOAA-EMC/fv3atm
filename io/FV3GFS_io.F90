...
!$omp parallel do default(shared) private(nb, ix, tem)
        do nb = 1, Atm_block%nblks
          do ix = 1, Atm_block%blksz(nb)
            tem = (one-Sfcprop(nb)%landfrac(ix)) * Sfcprop(nb)%fice(ix)
            if (Sfcprop(nb)%fice(ix) > min(Model%min_seaice,Model%min_lakeice) .and. tem > zero) then
              Sfcprop(nb)%snodi(ix) = Sfcprop(nb)%snowd(ix) / tem   !--- use snowd over ice and zero over water/land
            else
